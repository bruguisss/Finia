...
export default function MobileMoreSheet({ currentPage, onNavigate, onUpload, onClose }) {
  return (
    <div className="md:hidden fixed inset-0 z-50">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-elevated/95 backdrop-blur-xl backdrop-saturate-150 border-t border-white/10 rounded-t-2xl pb-[max(0.75rem,env(safe-area-inset-bottom))] animate-slide-up">
        <div className="w-9 h-1 bg-white/15 rounded-full mx-auto mt-2.5 mb-1" />
        <div className="px-2 py-1">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[14px] font-medium transition-colors duration-150 ${
                  active ? 'text-primary bg-white/[0.06]' : 'text-secondary hover:text-primary'
                }`}
              >
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[14px] font-medium text-secondary hover:text-primary transition-colors duration-150"
          >
